...
public class STVisitor implements BasicLVisitor
{
  ...
  public Object visit(ASTProgram node, Object data)
  {
    ...
    while(iter.hasNext())
    {
      ...
      while(iter2.hasNext())
      {
        ...
        System.out.print("| DataType: "+temp.getDataType());
        System.out.print("| Type: "+temp.getType().image);
        System.out.println("| Values: "+temp.getValues()); 
      } 
    }
  ...
  public Object visit(ASTAssign node, Object data)
  {
    ...
    if(lhSTC != null)
    {
      if(lhSTC.getDataType() == DataType.VAR || lhSTC.getDataType() == DataType.PARAMVAR)
      {
        lhType = lhSTC.getType().image;
          ...
          if(node.jjtGetChild(i).jjtAccept(this,null) instanceof Token )
          {
            ...
            else
            {
              if(t.kind == 31)
              {
                ...
                if(rhSTC.getType().image != lhType)
                {
                  System.out.println("Invalid type assign: \n\t"+lhToken.image+":"+lhType+" <- "+t.image+":"+rhSTC.getType().image);
                  System.out.println("Error Line: "+t.beginLine+" Column: "+t.beginColumn+"\n");
                  numErrors++;
                }
              }
      ...
      {
        System.out.println("Cannot Asign to constant: "+lhSTC.getName().image);
        System.out.println("Error Line: "+lhSTC.getName().beginLine+" Column: "+lhSTC.getName().beginColumn+"\n");
        numErrors++;
      }  
    }
    else{
  ...
  public void checkFunctionCall(ASTFunctionCall node, Object data)
  {
    ...
    if(hTemp != null)
    {
      ...
      while(iter.hasNext())
      { 
        if(hTemp.get(iter.next()).getDataType() == DataType.PARAMVAR)
          numParams ++;
      }
